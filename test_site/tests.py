...
class TestReport(TestCase):
    def setUp(self):
        self.user1 = UserProfile.create_user("reporter" "treportgmail.com", "testing123")
        self.user2 = UserProfile.create_user("reported", "reported@gmail.com", "testing456")
        self.post = Post.objects.create(user=self.user1, text="This is a post.")

    def test_reort_user(self):
        test_report = Report.objects.create(report_user=self.user1, reported_user= self.user2, reason="test reason")
        self.assertTrue(Report.objects.filter(report_id=test_report.report_id))


    def test_report_post(self):
        test_report_post = Report.objects.create(report_user=self.user1, reported_post=self.post, reason="test post reason")
        self.assertTrue(Report.objects.filter(report_id=test_report_post.report_id))

    def test_report_comment(self):
        comment = Comment.objects.create(post=self.post, user=self.user1, content="Test comment")
        test_report_comment = Report.objects.create(report_user=self.user1, reported_comment=comment, reason="test comment reason")
